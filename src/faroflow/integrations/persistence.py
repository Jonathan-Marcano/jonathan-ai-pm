...
from faroflow.security import redact_text
...
class IntegrationStateStore:
    ...
    def upsert_identity(
        ...
        *,
        ...
        if identity is not None:
            ...
            identity.external_name = _optional_text(external_name)
            identity.mime_type = _optional_text(mime_type)
            ...
            identity = ExternalIdentity(
                ...
                external_name=_optional_text(external_name),
                mime_type=_optional_text(mime_type),
    ...
    def list_deliverable_identities(
        self,
        *,
        source_system: str,
        limit: int | None = None,
        offset: int = 0,
    ) -> tuple[list[ExternalIdentity], bool]:
        """Return external identities linked to deliverables (Drive links) as (items, has_more)."""
        source_system = _required_text(source_system, "source_system").lower()
        statement = (
    ...
    def unlink_deliverable(self, deliverable_id: str) -> None:
        """Remove all external links for a deliverable without touching the external file."""
        statement = (
            select(ExternalIdentity)
            .where(
                ExternalIdentity.entity_kind == "deliverable",
                ExternalIdentity.entity_id == deliverable_id,
            )
        )
        identities = list(self.session.scalars(statement))
        if not identities:
            raise IntegrationStateError(f"deliverable has no drive link: {deliverable_id}")
    ...
    def list_runs(
        ...
        *,
        ...
        limit: int | None = None,
        offset: int = 0,
    ) -> tuple[list[SyncRun], bool]:
        """Return synchronization runs as (items, has_more) with optional filters."""
        statement = select(SyncRun)
        if source_system is not None:
            statement = statement.where(SyncRun.source_system == source_system.strip().lower())
        if resource_kind is not None:
            statement = statement.where(SyncRun.resource_kind == resource_kind.strip().lower())
        if status is not None:
            statement = statement.where(SyncRun.status == status.strip().lower())
        statement = statement.order_by(SyncRun.started_at.desc(), SyncRun.id).offset(offset)
        if limit is None:
            return list(self.session.scalars(statement)), False
        rows = list(self.session.scalars(statement.limit(limit + 1)))
        has_more = len(rows) > limit
    ...
    def find_project_mapping(
        self,
        *,
        source_system: str,
        external_scope: str = "",
        external_id: str,
    ) -> MeetingProjectMapping | None:
        source_system = _required_text(source_system, "source_system").lower()
        external_id = _required_text(external_id, "external_id")
        external_scope = external_scope.strip()
        statement = select(MeetingProjectMapping).where(
            MeetingProjectMapping.source_system == source_system,
            MeetingProjectMapping.external_scope == external_scope,
            MeetingProjectMapping.external_id == external_id,
        )
        return self.session.scalar(statement)
    ...
    def set_project_mapping(
        ...
        *,
        ...
        """Record a user-confirmed source key to project association."""
        source_system = _required_text(source_system, "source_system").lower()
        external_id = _required_text(external_id, "external_id")
        external_scope = external_scope.strip()
        ...
        mapping = self.find_project_mapping(
            source_system=source_system,
            external_scope=external_scope,
            external_id=external_id,
        )
        if mapping is None:
            mapping = MeetingProjectMapping(
                id=f"map_{uuid4().hex}",
                source_system=source_system,
                external_scope=external_scope,
                external_id=external_id,
        ...
        elif mapping.project_id != project_id:
    ...
    def list_project_mappings(
        ...
        limit: int | None = None,
        offset: int = 0,
    ) -> tuple[list[MeetingProjectMapping], bool]:
        statement = select(MeetingProjectMapping).order_by(
            MeetingProjectMapping.created_at, MeetingProjectMapping.id
        ).offset(offset)
        if limit is None:
            return list(self.session.scalars(statement)), False
        rows = list(self.session.scalars(statement.limit(limit + 1)))
    ...
    def delete_project_mapping(self, mapping_id: str) -> None:
        mapping = self.session.get(MeetingProjectMapping, mapping_id)
        if mapping is None:
            raise IntegrationStateError(f"Meeting project mapping not found: {mapping_id}")
