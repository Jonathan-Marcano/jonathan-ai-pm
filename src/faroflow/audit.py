...
from faroflow.models import MODEL_BY_KIND, AuditEvent, utc_now
...
def list_audit_events_page(
    ...
    *,
    ...
    """Return (events, has_more) honoring limit/offset for API pagination."""
    ...
    statement = statement.order_by(AuditEvent.occurred_at, AuditEvent.id).offset(offset)
    if limit is None:
        return list(session.scalars(statement)), False
...
def list_audit_events(
    ...
    *,
    ...
    events, _has_more = list_audit_events_page(
        session,
        entity_kind=entity_kind,
        entity_id=entity_id,
        actor=actor,
