...
| P4-02 | Must | Capture work over a supported business messaging integration | **Done** — a Telegram Bot adapter (`TelegramMessagingAdapter`) polls only scoped conversations and converts bounded updates into `ExternalInboundMessage` captures; `ingest_inbound_messages` lands them in the inbox as `BandejaItem` records through the inbound contract, deduped idempotently on `source_system + conversation_id + external_id`, and never auto-applies anything. The adapter reports HTTP 503 until tenant authorization is configured, outbound `send` stays blocked (P4-03) |
